...
class ImagesPipeline(FilesPipeline):
    ...
    def file_path(self, request, response=None, info=None):
        ...
        image_guid = url.replace('http://', '').replace(filename, '')
        #image_guid = hashlib.sha1(url).hexdigest()  # change to request.url after deprecation
        return '%sfull/%s.jpg' % (image_guid, filename.split('.')[0])
    ...
    def thumb_path(self, request, thumb_id, response=None, info=None):
        ...
        ## end of deprecation warning block
        filename = url.split('/')[-1]
        thumb_guid = url.replace('http://', '').replace(filename, '')
        #thumb_guid = hashlib.sha1(url).hexdigest()  # change to request.url after deprecation
        return '%sthumbs/%s/%s.jpg' % (thumb_guid, thumb_id, filename.split('.')[0])
